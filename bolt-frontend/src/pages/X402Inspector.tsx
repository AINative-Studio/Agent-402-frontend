...
import { getX402RequestsByRun } from '../lib/api';
import type { X402Request } from '../lib/types';

export function X402Inspector() {
  const { runId } = useParams<{ runId: string }>();
  const [requests, setRequests] = useState<X402Request[]>([]);
  ...
  const loadRequests = async () => {
    ...
    try {
      setLoading(true);
      const data = await getX402RequestsByRun(runId);
      setRequests(data);
    } catch (err) {
      console.error('Failed to load X402 requests:', err);
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <code className="text-sm font-mono">{truncate(request.did, 24)}</code>
                          <button
                            onClick={() => copyToClipboard(request.did)}
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex items-center gap-2">
                        <code className="text-sm font-mono">{truncate(request.signature, 48)}</code>
                        <button
